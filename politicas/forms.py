...
class IniciativaForm(forms.Form):
    tipo = forms.ModelChoiceField(queryset=TipoIniciativa.objects.all(),
                             required=False)
    tema = forms.ModelChoiceField(queryset=TemasAborda.objects.all(), 
                                  required=False)
